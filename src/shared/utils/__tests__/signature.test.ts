...
const LEGACY_CHECK_KEY = '80306f4370b39fd5630ad0529f77adb6'
...
describe('request signature', () => {
  ...
  it('stringToByte converts hex string (LEGACY_CHECK_KEY format)', () => {
  ...
  it('FuckingDSign returns a 32-char hex string', () => {
    const key = stringToByte(LEGACY_CHECK_KEY)
  ...
  it('FuckingDSign is deterministic for same index result', () => {
    ...
    const key1 = stringToByte(LEGACY_CHECK_KEY)
    const key2 = stringToByte(LEGACY_CHECK_KEY)
  ...
  it('FuckingDSign produces different results for different index paths', () => {
    const key1 = stringToByte(LEGACY_CHECK_KEY)
    const key2 = stringToByte(LEGACY_CHECK_KEY)
    const key3 = stringToByte(LEGACY_CHECK_KEY)
